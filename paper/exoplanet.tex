...
In particular, there are several open source packages that have been developed for fitting observations of exoplanets including, for example, \project{EXOFASTv2} \citep{Eastman:2019}, \project{juliet} \citep{Espinoza:2019}, \project{allesfitter} \citep{Gunther:2019}, \project{Exo-Striker} \citep{Trifonov:2019}, \project{pyaneti} \citep{Barragan:2019}, and \project{RadVel} \citep{Fulton:2018}.
...
With this contrast in mind, it might make more sense to compare \exoplanet\ to projects like \project{batman} \citep{Kreidberg:2015}, \project{ellc} \citep{Maxted:2016}, or \project{celerite} \citep{Foreman-Mackey:2017}, that work behind-the-scenes for many of these projects.
...
There are previous papers in the literature that present derivations of some of the methods included in \exoplanet---most notably, for transit light curves \citep[\project{starry},][]{Pal:2008, Luger:2019, Agol:2019} and for scalable Gaussian Processes \citep{Foreman-Mackey:2018a}---but \exoplanet\ is, to our knowledge, the first project to integrate these methods within a larger modeling framework for backpropagation, the topic of the following section.
...
\begin{eqnarray}
\dd\bvec{C} &=& \frac{\partial f}{\partial \bvec{A}}\,\dd\bvec{A} + \frac{\partial f}{\partial \bvec{B}}\,\dd\bvec{B} \eqlabel{c-partial}\\
...
Another derivative that we might care about is the derivative of the \emph{output} $y$ with respect to the input parameters $x$.
\begin{eqnarray}
\dd y &=& \mathrm{Tr}\left(\frac{\partial y}{\partial \bvec{C}}^\T\,\dd \bvec{C}\right)
\end{eqnarray}
Plugging in \eq{c-partial}, we find
\begin{eqnarray}
\dd y &=& \mathrm{Tr}\left(\frac{\partial y}{\partial \bvec{C}}^\T\,\frac{\partial f}{\partial \bvec{A}}\,\dd\bvec{A}\right) + \mathrm{Tr}\left(\frac{\partial y}{\partial \bvec{C}}^\T\,\frac{\partial f}{\partial \bvec{B}}\,\dd\bvec{B}\right)
\end{eqnarray}
